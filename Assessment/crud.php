...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $action === "update") {
    ...
    if ($id <= 0 || $first === "" || $last === "" || $email === "" || $contact === "" || $password === "") {
        $error = "Invalid input. All fields are required.";
    } else {
        $stmt = mysqli_prepare(
            $conn,
            "UPDATE students
                SET first_name = ?, last_name = ?, email = ?, contact = ?, password = ?
              WHERE id = ?"
        );
        mysqli_stmt_bind_param($stmt, "sssisi", $first, $last, $email, $contact, $password, $id);
...
if ($action === "edit") {
    ...
    if ($id > 0) {
        $stmt = mysqli_prepare(
            $conn,
            "SELECT id, first_name, last_name, email, contact, password FROM students WHERE id = ?"
        );
        mysqli_stmt_bind_param($stmt, "i", $id);
